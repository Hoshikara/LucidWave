...
function drawButton(button, x, y)
	...
	local buttonBorder = portrait and (2 * scale) or (1 * scale)
    ...
    gfx.RoundedRect(rx, ty - buttonBorder, buttonWidth, buttonHeight, portrait and 22 * scale or 15 * scale)
	gfx.Fill()
	gfx.Stroke()

	local shift = 1 * scale
	local shift2 = 2 * scale

    gfx.BeginPath()
    gfx.TextAlign(gfx.TEXT_ALIGN_CENTER + gfx.TEXT_ALIGN_MIDDLE)
	gfx.FontSize(portrait and math.floor(36 * scale) or math.floor (24 * scale))
	gfx.FillColor(125, 125, 125)
    gfx.Text(name, x + shift, y + shift)
	gfx.Fill()
...
function drawCursor(x, y, deltaTime)
	...
	gfx.RoundedRect(rx, ty, buttonWidth, buttonHeight, portrait and 22 * scale or 15 * scale)
	gfx.Fill()
	gfx.Stroke()
...
render = function(deltaTime)
	...
	buttonWidth = portrait and 190 * scale or 120 * scale
	buttonHeight = portrait and 45 * scale or 30 * scale
	...
	cursorGet = 1
    buttonY = portrait and (resy / 2) + (17 * scale) or (resy / 2) + (4 * scale)
    hovered = nil
	...
	for i=1, #buttons do
		...
		buttonY = portrait and buttonY + (27 * scale) or buttonY + (15 * scale)
