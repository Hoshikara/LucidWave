...
void main()
{
	...
	vec4 centerCol = texture(mainTex, clamp(ScaleUV(centerTexUV,portrait(3.8,1.5),vec2(0.5)),0.0,1.0)*0.25+vec2(0.14,0.0625));
	vec4 centerColClear = texture(mainTex, clamp(ScaleUV(centerTexUV,portrait(3.8,1.5),vec2(0.5)),0.0,1.0)*0.25+vec2(0.42,0.0625));
	...
	for(int i=0;i<3;++i){
        ...
        float spriteIndex = floor(rnd*2.0)*0.0625;
		...
		vec4 particle1 = texture(mainTex, particleUV*0.0625+particleOffset + vec2(spriteIndex+0.0625*2,0.0)).rgba * min(particleTime*2.0,1.0);
		particles = mix(particle,particle1,clearTransition);
	}
	...
	vec4 bgcolClear = texture(mainTex, bguvClear);
