...
void main()
{
	...
	centerUV.x -= 0.5;
	centerUV.y += portrait(0.14,0.25);
	centerUV.x *= ar;
	centerUV.x += 0.5;
	centerUV = clamp(centerUV,0.0,1.0);
	
	centerTexUV = rotate_point(vec2(0.5), clamp(BaseRotation + (bgrot * TWO_PI),-360,360) , centerUV);
	vec4 centerCol2 = texture(mainTex, clamp(ScaleUV(centerTexUV,portrait(1.5,1.0),vec2(0.5)),0.0,1.0)*0.25+vec2(0.42,0.0625));
	...
	vec4 particles = vec4(1.0,1.0,1.0,0.0);
	...
	target.rgb = mix(target.rgb,centerCol2.rgb + target.rgb,centerCol2.a * (0.2+abs(cos(timing.z)*0.2)));
