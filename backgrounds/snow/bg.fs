...
void main()
{
	...
	vec4 centerCol = texture(mainTex, clamp(ScaleUV(centerTexUV,portrait(2.5,1.5),vec2(0.5)),0.0,1.0)*0.25+vec2(0.14,0.0625));
	
	centerUV = screenUV;
	
    centerUV.x -= 0.5;
	centerUV.y += portrait(0.18,0.26);
	centerUV.y -= cos(timing.z) * 0.003;
	centerUV.x *= ar;
	centerUV.x += 0.5;
	centerUV = clamp(centerUV,0.0,1.0);
	
	centerTexUV = rotate_point(vec2(0.5), clamp(BaseRotation + (bgrot * TWO_PI),-360,360) , centerUV);
	vec4 centerCol2 = texture(mainTex, clamp(ScaleUV(centerUV,portrait(5,1.5),vec2(0.5)),0.0,1.0)*0.125+vec2(0.70,0.0625));
	
	centerUV = screenUV;
	
    centerUV.x -= 0.5;
	centerUV.y += portrait(0.14,0.29);
	centerUV /= portrait(1,1.2);
	centerUV.x *= ar;
	centerUV.x += 0.5;
	centerUV = clamp(centerUV,0.0,1.0);
	
	centerTexUV = rotate_point(vec2(0.5), clamp(BaseRotation + (bgrot * TWO_PI),-360,360) , centerUV);
	vec4 centerColClear = texture(mainTex, clamp(ScaleUV(centerTexUV,portrait(1.4,1),vec2(0.5)),0.0,1.0)*0.25+vec2(0.42,0.0625));
	...
	centerUV = screenUV;
	centerUV.x -= 0.5;
	centerUV.y -= portrait(-0.02,-0.8)+sin(timing.z) * 0.003;
	centerUV.x *= ar;
	centerUV /= 1.2;
	centerUV /= ar;
	centerUV.x += 0.5;
	...
	vec2 bguv2 = centerUV*0.46875+vec2(0.515625,0.515625);
	vec4 bgcol2 = texture(mainTex, bguv2);

	target.rgb = bgcol * (0.8+clearTransition/5);
    target.rgb = mix(target.rgb,centerCol2.rgb,centerCol2.a / 1.4);
	target.rgb = mix(target.rgb,bgcol2.rgb,bgcol2.a);
	target.rgb = mix(target.rgb,centerCol.rgb + target.rgb,centerCol.a / 1.5);
	target.rgb = mix(target.rgb,centerColClear.rgb,centerColClear.a * min(max(0,timing.z - 64),1));
